        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="breadcrumb__links">
                    <router-link :to="{name: 'category', params: { slug: slug_param}}"><i class="fa fa-home"></i> Trang chủ</router-link>
                    <router-link :to="{name: 'category', params: { slug: ''}}">Quốc gia</router-link>
                    <span>{{ category ? category.name : 'Tất cả' }}</span>
                </div>
            </div>
        </div>
    </div>
                        ...
                        <div class="row content-genre" v-if="slug_param == ''">
                            <div class="item-checkbox-genre" v-for="category in filter.categories" :key="category.id">
                                <article class="feature1">
                                    <input type="checkbox" name="checkCountry[]" :checked="category.slug == slug_param" :value="category.id"
...
export default {
    ...
    methods: {
        ...
        handleFilterData: function(pageNum = 1){
            ...
            if(!this.slug_param) {
                checkedCategories = this.$refs.categoryInputs.filter(input => input.checked).map(input => input.value)
            }
